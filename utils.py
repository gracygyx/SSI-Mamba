...
import torch
import numpy as np
from sklearn.metrics import roc_auc_score, precision_recall_curve, mean_squared_error, r2_score, auc, cohen_kappa_score
...
def metrics_graph(yt, yp):
    ...
    bacc = bacc_list[max_index]

    yp = np.array(yp)  # 将yp转换为NumPy数组
    yt = np.array(yt)
    thresholds = np.array(thresholds).flatten()
    yt_binary = (yt > 0.5).astype(int)
    yp_binary = (yp > thresholds[max_index]).astype(int)
    kappa = cohen_kappa_score(yt_binary, yp_binary)
    return auc, aupr, f1_score[0, 0], accuracy[0, 0], recall[0, 0], precision[0, 0], kappa, bacc[0, 0]
...
def get_ECFP_with_counts(smiles, radius=3, nBits=2048):
    """
    计算给定 SMILES 的计数型 ECFP 指纹（默认为 ECFP-6，即 radius=3）。

    Args:
        smiles (str): SMILES 字符串。
        radius (int): ECFP 半径，默认值为 3（即 ECFP-6）。
        nBits (int): 指纹的位数，默认为 2048。

    Returns:
        np.ndarray: 指纹的 NumPy 表示，形状为 [nBits]。
    """
    # 将 SMILES 转换为分子对象
    ...
    # 使用 AllChem.GetHashedMorganFingerprint 生成计数型 ECFP 指纹
